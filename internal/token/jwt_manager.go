...
func (maker *JWTMaker) CreateToken(id int64, name string, duration time.Duration) (string, *UserClaims, error) {
...
func (maker *JWTMaker) VerifyToken(tokenStr string) (*UserClaims, error) {
	token, err := jwt.ParseWithClaims(tokenStr, &UserClaims{}, func(token *jwt.Token) (interface{}, error) {
		// verify the signing method stated in JWT header. Used to secure token from method changing
